...
export const VendorPriceModal: React.FC<VendorPriceModalProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-2xl space-y-5 sm:space-y-6 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-3 sm:pb-4 gap-3">
        <div>
          <h2 className="text-base sm:text-lg font-bold text-white flex items-center space-x-2">
            <Tag className="w-5 h-5 text-indigo-400 flex-shrink-0" />
            <span>Vendor Quotations & Pricing Master</span>
          </h2>
        ...
        <div className="flex items-center gap-2 sm:gap-3 justify-between sm:justify-end">
          ...
          >
            ...
            <span>{showAdd ? 'Hide Form' : 'Add Price'}</span>
          </button>
      ...
      {showAdd && (
        <form onSubmit={handleSave} className="bg-slate-950 border border-slate-800 p-3 sm:p-4 rounded-xl space-y-3 sm:space-y-4">
